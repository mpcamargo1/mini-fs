...
/*Devolve i -> posiçao da string comando que contem última pasta/arquivo digitado*/
int string_tokenizer(){
...
void print_ajuda(){
  ...
  fprintf(stderr, "\nremoved [\\caminho]\\dir          ---> remover diretório(se estiver vazio)");
  fprintf(stderr, "\nremovea [\\caminho] arquivo       ---> remover arquivo");
  fprintf(stderr, "\nverd [\\caminho]\\dir             ---> exibir diretório");
  fprintf(stderr, "\nverset [\\caminho]\\arquivo       ---> exibir setores ocupados pelo arquivo");
...
void imprimir_shell(dir *mapadiretorio){
  ...
  fprintf(stderr,"%sFS:\\%s %s>",ANSI_COLOR_GREEN,mapadiretorio->nome,ANSI_COLOR_RESET);
...
void criar_arquivo(dir *mapadiretorio, char *nome, int data){
   ...
   if(mapadiretorio->arquivodir == NULL){
    strncpy(arquivo->nome,nome,sizeof(arquivo->nome));
	  strftime(arquivo->date_time, sizeof (arquivo->date_time), "%d/%m/%Y %H:%M:%S", ptm);
    ...
    fprintf(stderr,"Arquivo %s salvo com sucesso 1ºSetor : %d \n", arquivo->nome,arquivo->setor);
	      ...
	      arquivodisk *arquivo_disk = (arquivodisk*)malloc(sizeof(arquivodisk));
	 	  ...
	 	  strftime(arquivo_disk->date_time, sizeof (arquivo_disk->date_time), "%d/%m/%Y %H:%M:%S", ptm);
        ...
        arquivo_disk->proxsetor = NULL; 
	      ...
	      fprintf(stderr,"Arquivo %s salvo com sucesso 1ºSetor : %d \n", arquivo_disk->nome,arquivo_disk->setor);
	    	...
	    	fprintf(stderr,"ERRO: Não foi possível criar o arquivo\nJá existe um arquivo com o mesmo nome !\n");
   }
...
void exibe_arvore(dir *mapadiretorio, int rcont){ 
  //rcont = contador de recursao, para poder colocar os espaçamentos corretos(apenas visual)
    ...
    */
    ...
    while(dir_aux !=NULL){
      ...
      for(int i=0; i<rcont;i++)
        fprintf(stderr, "|  ");
      fprintf(stderr,"%s%s%s\n",ANSI_COLOR_BLUE, dir_aux->nome, ANSI_COLOR_RESET);
...
void executarcomando(dir *mapadiretorio){
         ...
         if(strcmp(comando_shell[0],SAIR) == MATCH){
            ...
            if(novo != NULL){
              ...
              fprintf(stderr,"Dica : Somente é permitido criar diretórios dentro da raiz\n");
...
void remover_arquivo(dir *pasta_nivel, dir*pasta_nivel_anterior,char *nome_arquivo){
...
void remover_arquivo_disco(arquivodisk *arquivo){
 ...
 /*Apaga o primeiro setor do arquivo*/
  ...
  while(aux_setor != NULL){
    /*Apaga o setor*/
      for(int i=aux_setor->inicio;i<(aux_setor->inicio + aux_setor->tam);i++){
        mydisk[aux_setor->setor].data[i] = 0; 
      }
    atualizar_mapabits(aux_setor->setor);   /*Atualiza o mapa de bits do setor que estava o arquivo*/
...
void remover_diretorio(dir *pasta_nivel, dir*pasta_nivel_anterior){
    ...
    /*Alocando memória para as estruturas auxiliares que percorrem o mapa de diretórios*/
   dir *aux = (dir*)malloc(sizeof(dir));
  ...
  if(pasta_nivel->proxnivel == NULL && pasta_nivel->arquivodir == NULL){
    ...
    if(strcmp(aux->nome,pasta_nivel->nome) ==  MATCH){
      mapa_bits[aux->posicao_setor] = 0;                /*Atualiza o mapa de bits daquele setor*/
      escrever_disco_diretorio(aux->posicao_setor,0);   /*Escreve no disco o diretório*/
      pasta_nivel_anterior->proxnivel = aux->irmao;     /*A pasta do nivel anterior aponta para o irmao da pasta deletada*/
      free(aux);                                        /*Libera recursos da estrutura aux*/
    }
    else{
      ...
      mapa_bits[aux->posicao_setor] = 0;              /*Atualiza o mapa de bits daquele setor*/
      escrever_disco_diretorio(aux->posicao_setor,0); /*Escreve no disco o diretório*/
      aux2->irmao = aux->irmao;                       /*A pasta anterior aponta para o irmao da pasta deletada*/
      free(aux);                                      /*Libera recursos da estrutura aux*/
    }
  }
  else{
...
/*Pesquisa o mapa de bits e o disco e grava o arquivo no disco o arquivo nos espaços vazios*/
int pesquisar_mapa_bits_arquivo(int data_total,arquivodisk *arquivo){
            ...
            bytes = 0;
              if(mydisk[i].data[k] == 0){
                fprintf(stderr,"Setor : %d Byte : %d\n",i,k);
                  ...
                  bytes++;
                  k++;
                  while(mydisk[i].data[k] == 0 && bytes != data_total && k<BLOCK_SIZE){
                    fprintf(stderr,"Setor : %d Byte : %d\n",i,k);
                        bytes++;
                        k++;
                  }
                  fprintf(stderr,"Setor : %d  Inicio : %d Fim : %d\n",i,inicio,inicio + bytes-1);
                  data_total = data_total - bytes;
                  escrever_disco_arquivo(i,bytes,inicio,arquivo,iteracao);
                  iteracao++;
...
void escrever_disco_arquivo(int bloco, int bytes,int inicio, arquivodisk *arquivo, int iteracao){
...
void verset(dir *diretorio, char *nome_arquivo){
  ...
  if(diretorio->arquivodir == NULL){
    ...
    while(aux != NULL){
      if(strcmp(aux->nome,nome_arquivo) == MATCH){
        fprintf(stderr, "Setor : %d Inicio : %d Fim : %d \n", aux->setor,aux->inicio,aux->inicio + aux->tam - 1);
        setor *aux_setor = (setor*)malloc(sizeof(setor));
        aux_setor = aux->proxsetor;
         while(aux_setor != NULL){
          fprintf(stderr, "Setor : %d Inicio : %d Fim : %d \n", aux_setor->setor,aux_setor->inicio,aux_setor->inicio + aux_setor->tam - 1);
          aux_setor = aux_setor->proxsetor;
...
void verd(dir *pasta){
    ...
    if(aux->arquivodir != NULL){
      ...
      while(arquivo_disk !=NULL){
         ...
         fprintf(stderr,"--%s %d bytes %s \n",arquivo_disk->nome, arquivo_disk->tam, arquivo_disk->date_time);
         arquivo_disk = arquivo_disk->proxarquivo;
      }
       
    }
    
    fprintf(stderr, "\n%d arquivo(s) e um tamanho de: %d byte(s)",arq_cont,tam_total);  
    fprintf(stderr, "\n%d subdiretório(s)\n",dir_cont);
